...
const NumberofCards=()=>{
    ...
    return(
        <div className=" md:flex">
            <h1 className="w-1/2 md:w-fit border border-blue-950 text-black my-1 md:m-2 md:p-1 font-bold rounded-lg bg-blue-300 shadow-md shadow-blue-700 hover:bg-blue-400" onClick={()=>{
                setshownumberofcards(!shownumberofcards)
            }}>Number Of Cards {shownumberofcards?'◀️':'▶️'}</h1>

            {shownumberofcards && (<div className="w-fit border  border-blue-950 rounded-lg  py-1 px-1 mt-2 mb-2 md:mt-auto md:mb-auto font-bold text-black"> <input className="mx-0.5" type="checkbox" value={cardnumber} onClick={()=>{
                setcardnumber(5);
            }}/> 5
