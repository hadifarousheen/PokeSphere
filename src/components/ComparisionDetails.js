...
const ComparisionDetails = ({ pokemondata }) => {
  ...
  const removeFromComparision = (id, pokemonName) => {
    ...
    const localStorageComparision = JSON.parse(
      localStorage.getItem("comparisions")
    );
    const updatedlocalStorageComparisions = localStorageComparision.filter(
      (poke) => poke.id !== id
    );
    localStorage.setItem(
      "comparisions",
      JSON.stringify(updatedlocalStorageComparisions)
    );
  ...
  return (
    <div className="mx-2 w-fit p-3 rounded-2xl shadow-2xl shadow-blue-800 mt-4 hover:border hover:border-blue-950 text-blue-950">
      ...
      <span className="text-2xl font-bold float-right text-blue-950">
        {name}
      </span>
      <img className="m-auto h-30" src={front_shiny} />
      <h1 className="text-xl font-bold hover:underline">Sprites</h1>
      <div className="flex">
        <img className="m-auto h-30" src={front_default} />
        <img className="m-auto h-30" src={back_shiny} />
        <img className="m-auto h-30" src={back_default} />
      </div>
